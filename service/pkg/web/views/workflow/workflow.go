...
import (
	...
	impl "github.com/scienceol/opensdl/service/pkg/core/workflow/workflow"
...
// 更新模板
func (w *Handle) NodeTemplateUpdate(ctx *gin.Context) {
	req := &workflow.NodeTemplateUpdateReq{}
	...
	err := w.wService.NodeTemplateUpdate(ctx, req)
...
func (w *Handle) GetWorkflowGraph(ctx *gin.Context) {
	req := &workflow.NodeTemplateReq{}
	if err := ctx.ShouldBindUri(req); err != nil {
		common.ReplyErr(ctx, code.ParamErr.WithMsg(err.Error()))
		return
	...
	if res, err := w.wService.GetWorkflowGraph(ctx, req.UUID); err != nil {
		common.ReplyErr(ctx, err)
	} else {
		common.ReplyOk(ctx, res)
	}
...
func (w *Handle) DeviceList(ctx *gin.Context) {
	req := &workflow.DeviceReq{}
	if err := ctx.ShouldBindQuery(req); err != nil {
		common.ReplyErr(ctx, code.ParamErr.WithMsg(err.Error()))
		return
	}

	res, err := w.wService.LabDeviceList(ctx, req)
	common.Reply(ctx, err, res)
}
