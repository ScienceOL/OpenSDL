...
type MaterialNode struct {
	...
	ReagentID      int64                    `gorm:"type:bigint" json:"reagent_id"` // 试剂 id
	...
	Color                string                `gorm:"-"`
	ParentUUID           uuid.UUID             `gorm:"-"`
	ResourceTemplateUUID uuid.UUID             `gorm:"-"`
}

func (m *MaterialNode) AfterFind(tx *gorm.DB) (err error) {
	m.Icon = config.Global().Dynamic().Storage.Image(m.Icon)
	return nil
}
