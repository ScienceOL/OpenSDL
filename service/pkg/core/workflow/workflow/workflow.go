...
func (w *workflowImpl) NodeTemplateDetail(ctx context.Context, templateUUID uuid.UUID) (*workflow.NodeTemplateDetailResp, error) {
	...
	return &workflow.NodeTemplateDetailResp{
		UUID:        template.UUID,
		Name:        utils.Or(template.DisplayName, template.Name),
...
func (w *workflowImpl) NodeTemplateUpdate(ctx context.Context, req *workflow.NodeTemplateUpdateReq) error {
	if req.UUID.IsNil() {
	...
	return w.materialStore.UpdateData(ctx, &model.WorkflowNodeTemplate{
...
func (w *workflowImpl) TemplateList(ctx context.Context, req *workflow.TplPageReq) (*common.PageResp[[]*workflow.TemplateListResp], error) {
	...
	handleMap := utils.Slice2MapSlice(handles, func(h *model.WorkflowHandleTemplate) (int64, *model.WorkflowHandleTemplate, bool) {
	...
	respList := utils.FilterSlice(templates.Data, func(t *model.WorkflowNodeTemplate) (*workflow.TemplateListResp, bool) {
		...
		return &workflow.TemplateListResp{
			UUID:      t.UUID,
			Name:      utils.Or(t.DisplayName, t.Name), // 模板名称（从device_action name字段取）
...
func (w *workflowImpl) OnWSMsg(ctx context.Context, s *melody.Session, b []byte) error {
	...
	case workflow.FetchToolTemplate:
		data, err = w.fetchToolNodeTemplate(ctx, s)
...
func (w *workflowImpl) fetchGraph(ctx context.Context, s *melody.Session) (any, error) {
	...
	if err := w.materialStore.FindDatas(ctx, &resourceNodeTemplates, map[string]any{
		"lab_id":        resp.Workflow.LabID,
	...
	nodes := utils.FilterSlice(resp.Nodes, func(node *repo.WorkflowNodeInfo) (*workflow.WSNode, bool) {
		...
		if node.Action != nil {
			...
			schema, defaultValue := w.handleSchema(ctx, materialNodes, resourceNodeTemplates, node.Action.Schema)
			data.Schema = schema
...
// GetWorkflowGraph 获取工作流 DAG 图（HTTP 接口）
func (w *workflowImpl) GetWorkflowGraph(ctx context.Context, workflowUUID uuid.UUID) (*workflow.WSGraph, error) {
	userInfo := auth.GetCurrentUser(ctx)
	resp, err := w.workflowStore.GetWorkflowGraph(ctx, userInfo.ID, workflowUUID)
	if err != nil {
		return nil, err
	...
	nodes := utils.FilterSlice(resp.Nodes, func(node *repo.WorkflowNodeInfo) (*workflow.WSNode, bool) {
		...
		if node.Action != nil {
			data.Name = utils.Or(node.Node.Name, node.Action.Name)
			schema, defaultValue := w.handleSchema(ctx, materialNodes, nil, node.Action.Schema)
			data.Schema = schema
			if len(data.Param) == 0 && len(defaultValue) != 0 {
				data.Param, _ = json.Marshal(defaultValue)
			} else if len(data.Param) != 0 && len(defaultValue) != 0 {
...
func (w *workflowImpl) fetchNodeTemplate(ctx context.Context, s *melody.Session) (any, error) {
	// 根据uuid查询workflow表，获取lab_id
	data, err := w.getWorkflow(ctx, s)
	if err != nil {
		return nil, err
	}

	// 根据lab_id查询workflow_node_template表，获取到数据后，去重，获取resource_node_id
	resp, err := w.workflowStore.GetWorkflowNodeTemplate(ctx, map[string]any{
		"lab_id":    data.LabID,
		"node_type": string(model.WorkflowNodeILab),
	})
	if err != nil {
		return nil, err
	}
	...
	// 根据resource_node_id查询resource_node_template表，获取id, uuid, name, config_schema
	...
	// 获取workflow_node_template表的主键id
	actionIDs := utils.FilterSlice(resp, func(item *model.WorkflowNodeTemplate) (int64, bool) {
		return item.ID, true
	})

	// 根据workflow_node_template表的id（workflow_handle_template表里的workflow_node_id）
	// 查询workflow_handle_template表，
	respHandles, err := w.workflowStore.GetWorkflowHandleTemplates(ctx, actionIDs)
	if err != nil {
		return nil, err
	}

	respActionMap := utils.Slice2MapSlice(resp, func(item *model.WorkflowNodeTemplate) (int64, *model.WorkflowNodeTemplate, bool) {
		return item.ResourceNodeID, item, true
	})

	respHandleMap := utils.Slice2MapSlice(respHandles, func(item *model.WorkflowHandleTemplate) (int64, *model.WorkflowHandleTemplate, bool) {
			...
			HandleTemplates: utils.FilterSlice(respActionMap[id], func(item *model.WorkflowNodeTemplate) (*workflow.WSTemplateHandles, bool) {
				return &workflow.WSTemplateHandles{
					Template: &workflow.WSTemplate{
						UUID:          item.UUID,
						Name:          utils.Or(item.DisplayName, item.Name),
						DisplayName:   utils.Or(item.DisplayName, item.Name),
						Header:        item.Class,
						Footer:        &item.Name,
						Schema:        item.Schema,
						ExecuteScript: "",
...
// 获取工具模板
func (w *workflowImpl) fetchToolNodeTemplate(ctx context.Context, s *melody.Session) (any, error) {
	// 根据uuid查询workflow表，获取lab_id
	data, err := w.getWorkflow(ctx, s)
	if err != nil {
		return nil, err
	...
	resp, err := w.workflowStore.GetWorkflowNodeTemplate(ctx, map[string]any{
		"lab_id":    data.LabID,
		"node_type": string(model.WorkflowToolCall),
	})
	if err != nil {
		return nil, err
	}
	...
	// 根据workflow_node_template表的id，查询workflow_handle_template表（workflow_handle_template表里的workflow_node_id）
	respHandles, err := w.workflowStore.GetWorkflowHandleTemplates(ctx, actionIDs)
	if err != nil {
		return nil, err
	}
	...
	// 生成workflow_node_id到workflow_handle_template表数据的映射
	respHandleMap := utils.Slice2MapSlice(respHandles, func(item *model.WorkflowHandleTemplate) (int64, *model.WorkflowHandleTemplate, bool) {
		return item.WorkflowNodeID, item, true
	})
				...
				return &workflow.WSTemplateHandles{
					...
					Handles: utils.FilterSlice(respHandleMap[item.ID], func(h *model.WorkflowHandleTemplate) (*workflow.WSNodeHandle, bool) {
						return &workflow.WSNodeHandle{
							...
							HandleKey:   h.HandleKey,
							IoType:      h.IoType,
							DisplayName: utils.Or(h.DisplayName, h.HandleKey),
							Type:        h.Type,
							DataSource:  h.DataSource,
							DataKey:     h.DataKey,
						}, true
...
func (w *workflowImpl) createNode(ctx context.Context, s *melody.Session, b []byte) (any, error) {
	...
	nodeData := &model.WorkflowNode{
		...
		Name:           utils.Or(reqData.Name, deviceAction[0].DisplayName, deviceAction[0].Name),
		Status:         "draft",
		Type:           utils.Or(reqData.Type, model.WorkflowNodeType(deviceAction[0].NodeType)),
	...
	schema, defaultValue := w.handleSchema(ctx, materialNodes, nil, deviceAction[0].Schema)
...
func (w *workflowImpl) runWorkflow(ctx context.Context, s *melody.Session, b []byte) (any, error) {
	...
	if exists, err := w.rClient.Exists(ctx, utils.LabHeartName(labUUID)).Result(); err != nil ||
		exists == 0 {
	...
	err = w.workflowStore.ExecTx(ctx, func(txCtx context.Context) error {
		...
		taskUUID = task.UUID
		data := lab.ApiData[engine.WorkflowInfo]{
			ApiMsg: lab.ApiMsg{
				Action: lab.StartWorkflow,
			},
		...
		logger.Infof(ctx, "runWorkflow ============ data: %s", string(dataB))

		ret := w.rClient.LPush(ctx, utils.LabTaskName(labUUID), dataB)
		if ret.Err() != nil {
...
func (w *workflowImpl) HttpRunWorkflow(ctx context.Context, req *workflow.RunReq) (uuid.UUID, error) {
	...
	if exists, err := w.rClient.Exists(ctx, utils.LabHeartName(labUUID)).Result(); err != nil ||
		exists == 0 {
		return uuid.NewNil(), code.EdgeNotStartedErr
	}
...
func (w *workflowImpl) stopWorkflow(ctx context.Context, s *melody.Session, b []byte) (any, error) {
	...
	task := &model.WorkflowTask{}
	if err := w.workflowStore.GetData(ctx, task, map[string]any{
		"uuid": req.Data,
	}, "lab_id", "status"); err != nil {
		return nil, code.WorkflowTaskNotFoundErr.WithMsg("can not get workflow task")
	}
	labUUID := w.workflowStore.ID2UUID(ctx, &model.Laboratory{}, task.LabID)[task.LabID]
	if labUUID.IsNil() {
		return nil, code.LabNotFound.WithMsgf("can not get lab id: %d", task.LabID)
	}
	...
	data := lab.ApiControlData[lab.StopJobReq]{
	...
	err = w.workflowStore.ExecTx(ctx, func(txCtx context.Context) error {
		...
		dataB, _ := json.Marshal(data)
		ret := w.rClient.LPush(ctx, utils.LabControlName(labUUID), dataB)
		if ret.Err() != nil {
			return code.ParamErr.WithMsgf("push workflow redis msg err: %+v", ret.Err())
		}
...
func (w *workflowImpl) getMappingTemplate(ctx context.Context, sourceLabID, targetLabID int64, nodes []*model.WorkflowNode) (map[int64]int64, []*workflow.DuplicateError, error) {
	...
	}, "id", "name", "resource_node_id", "display_name"); err != nil {
		return nil, nil, err
	}
	...
	if len(diffMap) > 0 {
		...
		return nil, utils.Map2Slice(diffMap, func(key string, value *workflow.TplMapping) (*workflow.DuplicateError, bool) {
...
func (w *workflowImpl) ImportWorkflow(ctx context.Context, req *workflow.ImportReq) (*workflow.CreateResp, error) {
	...
	if userInfo.LabID != 0 {
		...
		labID = w.labStore.UUID2ID(ctx, &model.Laboratory{}, req.TargetLabUUID)[req.TargetLabUUID]
	}
	...
	handleIndex := utils.Slice2MapSlice(handles, func(h *model.WorkflowHandleTemplate) (int64, *model.WorkflowHandleTemplate, bool) {
		return h.WorkflowNodeID, h, true
	})
	...
	if err := w.workflowStore.ExecTx(ctx, func(txCtx context.Context) error {
		wk := &model.Workflow{UserID: userInfo.ID, LabID: labID, Name: newName}
...
func (w *workflowImpl) LabDeviceList(ctx context.Context, req *workflow.DeviceReq) (*workflow.DevicesResp, error) {
	// userInfo := auth.GetCurrentUser(ctx)
	labID := w.workflowStore.UUID2ID(ctx, &model.Laboratory{}, req.LabUUID)[req.LabUUID]
	if labID == 0 {
		return nil, code.LabNotFound
	}
	// TODO: 校验该用户是否有权限查看

	deviceDatas := make([]*model.MaterialNode, 0)
	if err := w.workflowStore.FindDatas(ctx, &deviceDatas, map[string]any{
		"lab_id":           labID,
		"type":             model.MATERIALDEVICE,
		"resource_node_id": []any{"$ne", 0},
	}, "id", "name", "uuid", "parent_id", "resource_node_id"); err != nil {
		return nil, err
	}

	deviceTplsID := utils.FilterUniqSlice(deviceDatas, func(d *model.MaterialNode) (int64, bool) {
		return d.ResourceNodeID, true
	})
	deviceTplsUUIDMap := w.workflowStore.ID2UUID(ctx, &model.ResourceNodeTemplate{}, deviceTplsID...)

	resources := make([]*model.ResourceNodeTemplate, len(deviceTplsID))
	if err := w.workflowStore.FindDatas(ctx, &resources, map[string]any{
		"id":     deviceTplsID,
		"lab_id": labID,
	}, "id", "uuid", "name"); err != nil {
		return nil, err
	...
	// 获取所有的 action
	wkNodeTpls := make([]*model.WorkflowNodeTemplate, 0)
	if err := w.workflowStore.FindDatas(ctx, &wkNodeTpls, map[string]any{
		"lab_id":           labID,
		"resource_node_id": deviceTplsID,
	}); err != nil {
		return nil, err
	}

	wkNodeIDs := utils.FilterSlice(wkNodeTpls, func(t *model.WorkflowNodeTemplate) (int64, bool) {
		return t.ID, true
	})

	// 获取所有的action handle
	wkNodeHandleTpls := make([]*model.WorkflowHandleTemplate, 0)
	if err := w.workflowStore.FindDatas(ctx, &wkNodeHandleTpls, map[string]any{
		"workflow_node_id": wkNodeIDs,
	}); err != nil {
		return nil, err
	}

	wkNodeHandleTplGroup := utils.Slice2MapSlice(wkNodeHandleTpls, func(h *model.WorkflowHandleTemplate) (int64, *model.WorkflowHandleTemplate, bool) {
		return h.WorkflowNodeID, h, true
	})

	devices := utils.FilterSlice(deviceDatas, func(d *model.MaterialNode) (*workflow.DeviceNode, bool) {
		return &workflow.DeviceNode{
			UUID:         d.UUID,
			Name:         d.Name,
			TemplateUUID: deviceTplsUUIDMap[d.ResourceNodeID],
		}, true
	})

	wkNodeTplGroup := utils.Slice2MapSlice(wkNodeTpls, func(n *model.WorkflowNodeTemplate) (int64, *model.WorkflowNodeTemplate, bool) {
		return n.ResourceNodeID, n, true
	})

	return &workflow.DevicesResp{
		Devices: devices,
		DeviceTemplates: utils.Slice2MapCondition(resources, func(r *model.ResourceNodeTemplate) (uuid.UUID, *workflow.TplHandle, bool) {
			nodeTpls, ok := wkNodeTplGroup[r.ID]
			if !ok {
				return uuid.NewNil(), nil, false
			}

			return r.UUID, &workflow.TplHandle{
				HandleTemplates: utils.FilterSlice(nodeTpls, func(n *model.WorkflowNodeTemplate) (*workflow.WSTemplateHandles, bool) {
					wkHandles, ok := wkNodeHandleTplGroup[n.ID]
					if !ok {
						return nil, false
					}
					...
					return &workflow.WSTemplateHandles{
						Template: &workflow.WSTemplate{
							UUID:          n.UUID,
							Name:          n.Name,
							DisplayName:   utils.Or(n.DisplayName, n.Name),
							Header:        n.Class,
							Footer:        &n.Name,
							Schema:        n.Schema,
							ExecuteScript: "",
						...
						Handles: utils.FilterSlice(wkHandles, func(h *model.WorkflowHandleTemplate) (*workflow.WSNodeHandle, bool) {
							return &workflow.WSNodeHandle{
								UUID:        h.UUID,
								HandleKey:   h.HandleKey,
								IoType:      h.IoType,
