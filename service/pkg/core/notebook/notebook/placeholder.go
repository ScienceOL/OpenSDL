...
func (s *schemaHelper) handleSchema(ctx context.Context, materialNodes []*model.MaterialNode, resourceNodeTemplates []*model.ResourceNodeTemplate, schema datatypes.JSON) (datatypes.JSON, map[string]any) {
	...
	for fieldName, placeholderType := range placeholder {
		...
		case "unilabos_resource_amounts":
			keyDefault, uiSchema = s.handleResourceAmount(ctx)
		case "unilabos_resource_names":
			keyDefault, uiSchema = s.handleResourceName(ctx)
		case "unilabos_transfer_liquid":
			keyDefault, uiSchema = s.handleTransferLiquid(ctx, materialNodes, innerProperties, fieldName)
		case "unilabos_class":
			keyDefault, uiSchema = s.handleClass(ctx, resourceNodeTemplates, innerProperties, fieldName)
		default:
...
func (s *schemaHelper) handleClass(ctx context.Context, resourceNodeTemplates []*model.ResourceNodeTemplate, dataSchema map[string]any, key string) (map[string]any, map[string]any) {
	nodeMap := utils.Slice2Map(resourceNodeTemplates, func(node *model.ResourceNodeTemplate) (int64, *model.ResourceNodeTemplate) {
		return node.ID, node
	})

	labels := utils.Map2Slice(nodeMap, func(id int64, node *model.ResourceNodeTemplate) (*Label, bool) {
		return &Label{
			Value: node.Name,
			UUID:  node.UUID,
			Label: node.Name,
	...
	uiSchema := s.updateFieldWithOptions(dataSchema, key, labels, "请选择资源模板", "选择资源模板实例", true)
	if len(labels) == 1 {
...
func (s *schemaHelper) handleTransferLiquid(ctx context.Context, materialNodes []*model.MaterialNode, dataSchema map[string]any, key string) (map[string]any, map[string]any) {
	...
	labels := utils.Map2Slice(nodeMap, func(id int64, node *model.MaterialNode) (*Label, bool) {
	...
	uiSchema := s.updateFieldWithOptions(dataSchema, key, labels, "请选择资源", "选择资源实例", false)
	if len(labels) == 1 {
		return map[string]any{key: labels[0].Value}, uiSchema
	}
...
func (s *schemaHelper) handleNodes(ctx context.Context, materialNodes []*model.MaterialNode, dataSchema map[string]any, key string) (map[string]any, map[string]any) {
	...
	uiSchema := s.updateFieldWithOptions(dataSchema, key, labels, "请选择节点", "选择节点实例", false)
	if len(labels) == 1 {
		return map[string]any{key: labels[0].Value}, uiSchema
	}
	return nil, uiSchema
...
func (s *schemaHelper) handleResourceAmount(_ context.Context) (map[string]any, map[string]any) {
	return nil, map[string]any{
		"ui:widget":      "SampleAmountWidget",
		"ui:placeholder": "样品数量",
	}
}

func (s *schemaHelper) handleResourceName(_ context.Context) (map[string]any, map[string]any) {
	return nil, map[string]any{
		"ui:widget":      "SampleNameWidget",
		"ui:placeholder": "样品名称",
	}
}
...
func (s *schemaHelper) handleDevices(ctx context.Context, materialNodes []*model.MaterialNode, dataSchema map[string]any, key string) (map[string]any, map[string]any) {
	...
	uiSchema := s.updateFieldWithOptions(dataSchema, key, labels, "请选择设备", "选择设备实例", false)
	if len(labels) == 1 {
		return map[string]any{key: labels[0].Value}, uiSchema
	}
	return nil, uiSchema
...
func (s *schemaHelper) handleResources(ctx context.Context, materialNodes []*model.MaterialNode, dataSchema map[string]any, key string) (map[string]any, map[string]any) {
	...
	uiSchema := s.updateFieldWithOptions(dataSchema, key, labels, "请选择资源", "选择资源实例", true)
	if len(labels) == 1 {
		return map[string]any{key: labels[0].Value}, uiSchema
	}
	return nil, uiSchema
...
func (s *schemaHelper) updateFieldWithOptions(properties map[string]any, fieldName string, options []*Label, placeholder, description string, isRes bool) map[string]any {
	_ = placeholder
	// 查找目标字段
	...
	case "object":
		...
		uiWidget := "select"
		if isRes {
			uiWidget = "SingleSampleSelectWidget"
		}
	...
	case "array":
		...
		case "string":
			...
			uiWidget := "ArraySelectWidget"
			if isRes {
				uiWidget = "MaterialTemplateWidget"
			}
			// UI配置
			return map[string]any{
				"ui:widget":      uiWidget,
