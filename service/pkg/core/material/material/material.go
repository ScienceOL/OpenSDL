...
func NewMaterial(ctx context.Context, wsClient *melody.Melody) material.Service {
	m := &materialImpl{
		...
		reagentStore:  rStore.NewReagentRepo(),
		wsClient:      wsClient,
		msgCenter:     events.NewEvents(),
		machine:       machineImpl.NewMachine(),
...
func (m *materialImpl) fetchGraph(ctx context.Context, s *melody.Session) (any, error) {
	...
	resReagentIDs := utils.FilterUniqSlice(nodes, func(n *model.MaterialNode) (int64, bool) {
		return n.ReagentID, n.ReagentID > 0
	})

	resColorDatas := make([]*model.Reagent, 0, len(resReagentIDs))
	if err = m.reagentStore.FindDatas(ctx, &resColorDatas, map[string]any{
		"id": resReagentIDs,
	}, "id", "color"); err != nil {
		return nil, err
	}
	resColorMap := utils.Slice2Map(resColorDatas, func(r *model.Reagent) (int64, string) {
		color := ""
		if r.Color != nil {
			color = *r.Color
		}
	...
	respNodes := utils.FilterSlice(nodes, func(nodeItem *model.MaterialNode) (*material.WSNode, bool) {
		...
		var tplName string
		var initParamSchema datatypes.JSON
		if resNodeTpl, ok := resNodeTplMap[nodeItem.ResourceNodeID]; ok {
			tplUUID = resNodeTpl.UUID
			tplName = resNodeTpl.Name
			initParamSchema = resNodeTpl.ConfigSchema
		}
		if len(initParamSchema) == 0 {
			initParamSchema = datatypes.JSON("{}")
		}
		...
		return &material.WSNode{
			...
			InitParamSchema: initParamSchema,
			...
			Color:           resColorMap[nodeItem.ReagentID],
...
func (m *materialImpl) createNode(ctx context.Context, s *melody.Session, b []byte) (any, error) {
	...
	// 批量查询 Reagent UUID -> ID 映射，避免在循环内多次查询
	reagentUUIDs := utils.FilterUniqSlice(resNodeChildrenTpl, func(tpl *model.ResourceNodeTemplate) (uuid.UUID, bool) {
		if info, ok := reqData.PlateReagentDatas[tpl.Name]; ok && !info.ReagentUUID.IsNil() {
			return info.ReagentUUID, true
		}
		return uuid.UUID{}, false
	})
	uuid2ID := make(map[uuid.UUID]int64)
	if len(reagentUUIDs) > 0 {
		uuid2ID = m.reagentStore.UUID2ID(ctx, &model.Reagent{}, reagentUUIDs...)
	}

	childrenMaterialNode := utils.FilterSlice(resNodeChildrenTpl, func(tpl *model.ResourceNodeTemplate) (*model.MaterialNode, bool) {
		var reagentID int64
		reagentInfo, exist := reqData.PlateReagentDatas[tpl.Name]
		if exist && !reagentInfo.ReagentUUID.IsNil() {
			reagentID = uuid2ID[reagentInfo.ReagentUUID]
			if reagentID == 0 {
				logger.Errorf(ctx, "createNode reagent not found err: %s", reagentInfo.ReagentUUID)
				return nil, false
			}
		}
	...
	resDatas = append(resDatas, &material.WSNode{
		...
		Pose:            mData.Pose,
		Model:           mData.Model,
		Icon:            config.Global().Dynamic().Storage.Image(mData.Icon),
...
func (m *materialImpl) notifyEdgeAdd(ctx context.Context, labUUID uuid.UUID, nodes []*material.WSNode) {
	...
	// 通知调度器
	m.notify(ctx, labUUID, lab.AddMaterial, addMaterials)
}

func (m *materialImpl) notify(ctx context.Context, labUUID uuid.UUID, action lab.ApiControlAction, uuids []*engine.MaterialUpdate) {
	if len(uuids) == 0 {
		return
	}

	heartName := utils.LabHeartName(labUUID)
	session, _ := m.rClient.Get(ctx, heartName).Result()

	data := lab.ApiControlData[any]{
	...
	ret := m.rClient.LPush(ctx, utils.LabControlName(labUUID), dataB)
	if ret.Err() != nil {
...
func (m *materialImpl) upateNode(ctx context.Context, s *melody.Session, b []byte) error {
	...
	if ancestorNode, err := m.materialStore.GetAncestors(ctx, req.Data.UUID); err == nil {
		...
		m.notify(ctx, labUUID.(uuid.UUID), lab.UpdateMaterial, []*engine.MaterialUpdate{
			{
				UUID:          req.Data.UUID,
...
func (m *materialImpl) batchDelNode(ctx context.Context, s *melody.Session, b []byte) (any, error) {
	...
	m.notify(ctx, labUUID.(uuid.UUID), lab.RemoveMaterial, updates)
...
func (m *materialImpl) GetResourceNodeTemplate(ctx context.Context, req *material.AllTemplateReq) (*material.ResourceTemplates, error) {
	userInfo := auth.GetCurrentUser(ctx)
	if userInfo == nil {
		return nil, code.UnLogin
	}
	...
	tplNodes := make([]*model.ResourceNodeTemplate, 0, 1)
	if err := m.envStore.FindDatas(ctx, &tplNodes, map[string]any{
		"lab_id":        labData.ID,
		"resource_type": "resource",
	}, "uuid", "name", "resource_type", "tags"); err != nil {
		return nil, err
	}

	tplDatas := utils.FilterSlice(tplNodes, func(nodeItem *model.ResourceNodeTemplate) (*material.ResourceTemplate, bool) {
		return &material.ResourceTemplate{
			UUID:         nodeItem.UUID,
			Name:         nodeItem.Name,
			Tags:         nodeItem.Tags,
