...
func (m *materialImpl) upsertNode(ctx context.Context, mountID int64, req *material.UpsertMaterialReq) ([]*material.UpsertMaterialResp, error) {
	...
	if err := m.envStore.FindDatas(ctx, &resourceNodes, map[string]any{
		"lab_id": labID,
		"name":   resTplNames,
	}, "id", "name", "icon", "model", "uuid"); err != nil {
		return nil, err
	}
	...
	updateMaterialDatas := make([]*model.MaterialNode, 0, 10)
	newMaterialDatas := make([]*model.MaterialNode, 10)
	if err = db.DB().ExecTx(ctx, func(txCtx context.Context) error {
		// 删除 root node 下的子节点
		if len(delUUIDs) > 0 {
		...
		for _, nodes := range levelNodes {
			...
			for _, n := range nodes {
				...
				if resInfo := resMap[n.Class]; resInfo != nil {
					data.ResourceNodeID = resInfo.ID
					data.ResourceTemplateUUID = resInfo.UUID
			...
			if len(updateDatas) > 0 {
				...
				updateMaterialDatas = append(updateMaterialDatas, updateDatas...)
			...
			if len(newDatas) > 0 {
				...
				newMaterialDatas = append(newMaterialDatas, newDatas...)
			}
	...
	resData := &common.WSData[*material.UpdateNodes]{
		WsMsgType: common.WsMsgType{Action: string(material.UpdateNodeResData), MsgUUID: uuid.NewV4()},
		Data: &material.UpdateNodes{
			Nodes: utils.FilterSlice(updateMaterialDatas, func(d *model.MaterialNode) (*material.WSNode, bool) {
				return &material.WSNode{
					UUID:              d.UUID,
					ParentUUID:        d.ParentUUID,
					Name:              d.Name,
					DisplayName:       utils.Or(d.DisplayName, d.Name),
					Description:       d.Description,
					Type:              d.Type,
					ResTemplateUUID:   d.ResourceTemplateUUID,
					ResTemplateName:   d.Class,
					InitParamData:     d.InitParamData,
					InitParamSchema:   d.Schema,
					Schema:            d.Schema,
					Data:              d.Data,
					PlateWellDatas:    make(map[string]datatypes.JSON),
					PlateReagentDatas: make(map[string]material.ReagentInfo),
					Status:            d.Status,
					Header:            utils.Or(d.DisplayName, d.Name),
					Pose:              d.Pose,
					Model:             d.Model,
					Icon:              config.Global().Dynamic().Storage.Image(d.Icon),
	...
	return utils.Map2Slice(nodeParentMap, func(key uuid.UUID, n *model.MaterialNode) (*material.UpsertMaterialResp, bool) {
...
func (m *materialImpl) upsertMaterialNode(ctx context.Context, req *material.UpsertMaterialReq) ([]*material.UpsertMaterialResp, error) {
	...
	return utils.Map2Slice(nodeMap, func(key uuid.UUID, n *model.MaterialNode) (*material.UpsertMaterialResp, bool) {
		return &material.UpsertMaterialResp{
			UUID:        key,
			CloudUUID:   n.UUID,
			Name:        n.Name,
...
func (m *materialImpl) EdgeQueryMaterial(ctx context.Context, req *material.MaterialQueryReq) (*material.MaterialQueryResp, error) {
	...
	if req.WithChildren {
		nodes, err = m.materialStore.GetDescendantsByUUIDs(ctx, req.UUIDS)
		if err != nil {
			return nil, err
		}
	} else {
	...
	parentIDs := utils.FilterUniqSlice(nodes, func(n *model.MaterialNode) (int64, bool) {
		return n.ParentID, n.ParentID > 0 && slices.Contains(req.UUIDS, n.UUID)
	})
	...
	parentNodeMap := utils.Slice2Map(append(parentNodes, nodes...), func(n *model.MaterialNode) (int64, uuid.UUID) {
		return n.ID, n.UUID
	})
