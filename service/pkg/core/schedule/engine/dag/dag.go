...
import (
	...
	mcpRepo "github.com/scienceol/opensdl/service/pkg/repo/mcp"
...
func (d *dagEngine) loadData(ctx context.Context) error {
	...
	nodes, err := utils.FilterSliceWithErr(allNodes, func(node *model.WorkflowNode) ([]*model.WorkflowNode, bool, error) {
		...
		if node.Type == model.WorkflowToolCall {
...
func (d *dagEngine) Type(ctx context.Context) engine.JobType {
	return engine.WorkflowJobType
}
...
func (d *dagEngine) Run() error {
	...
	d.updateTaskStatus(d.ctx, taskStatus, d.job.TaskID)
	d.boardMsg(d.ctx, data)
...
func (d *dagEngine) runNode(ctx context.Context, node *model.WorkflowNode, job *model.WorkflowNodeJob) error {
	...
	defer func() {
		...
		if err != nil {
			...
			} else if errors.Is(err, code.JobTimeoutErr) {
				data.Msg = "job timeout"
				data.Type = "warning"
				data.JobStatus = "failed"
	...
	if node.Type == model.WorkflowPyScript || node.Type == model.WorkflowToolCall {
		return err
	}
...
func (d *dagEngine) execNodeAction(ctx context.Context, node *model.WorkflowNode, job *model.WorkflowNodeJob) error {
	...
	case model.WorkflowToolCall: // 新增tool调用模式
		return d.toolCall(ctx, node, job)
	default:
		return code.UnknownWorkflowNodeTypeErr
	}
}

func (d *dagEngine) toolCall(ctx context.Context, node *model.WorkflowNode, job *model.WorkflowNodeJob) error {
	req := &repo.CallMCPRequest{}
	err := json.Unmarshal(node.Param, req)
	if err != nil {
	...
	// 调用 MCP
	ret, err := d.mcp.CallMCP(ctx, req)
	if err != nil {
		returnInfo.Suc = false
		returnInfo.Error = err.Error()
	} else {
		returnInfo.Suc = true
...
func (d *dagEngine) sendAction(_ context.Context, node *model.WorkflowNode, job *model.WorkflowNodeJob) error {
	...
	logger.Warnf(context.Background(), "send data: %s", string(b))
