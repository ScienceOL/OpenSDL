...
func (m *materialImpl) GetDescendantsByUUIDs(ctx context.Context, uuids []uuid.UUID) ([]*model.MaterialNode, error) {
	sql := `
WITH RECURSIVE node_tree AS (
    ...
    SELECT 
        ...
        parent.root_uuid
    FROM material_node child
    INNER JOIN node_tree parent ON child.parent_id = parent.id 
    WHERE parent.level < 100 -- 递归深度控制在 100 层
)
SELECT DISTINCT ON (mn.id)
    ...
    nt.root_uuid
FROM node_tree nt
INNER JOIN material_node mn ON nt.id = mn.id
ORDER BY mn.id, nt.level;
	`
	var results []*model.MaterialNode
	if err := m.DBWithContext(ctx).Raw(sql, uuids).Scan(&results).Error; err != nil {
		logger.Errorf(ctx, "GetDescendantsByUUIDs fail node uuid: %s, err: %+v", uuids, err)
		return nil, err
	}
	return results, nil
}
