...
func (w *workflowImpl) GetWorkflowGraph(ctx context.Context, userID string, workflowUUID uuid.UUID) (*repo.WorkflowGrpah, error) {
	...
	actionHandleMap := utils.Slice2MapSlice(actionHandles, func(item *model.WorkflowHandleTemplate) (int64, *model.WorkflowHandleTemplate, bool) {
...
func (w *workflowImpl) GetWorkflowTasks(ctx context.Context,
	...
	// 如果指定了实验室ID，则按实验室过滤
	query = query.Where("lab_id = ? and (notebook_group_id is null or notebook_group_id = 0)", req.Data.LabID)
...
func (w *workflowImpl) CreateSample(ctx context.Context, datas []*model.WorkflowNodeJobSample) error {
	if statement := w.DBWithContext(ctx).Create(datas); statement.Error != nil {
		logger.Errorf(ctx, "workflowImpl.CreateSample fail err: %+v", statement.Error)
		return code.CreateDataErr.WithMsg(statement.Error.Error())
	}
