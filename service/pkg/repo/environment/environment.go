...
func (e *envImpl) GetResourceHandleTemplates(ctx context.Context, resourceNodeIDs []int64) (map[int64][]*model.ResourceHandleTemplate, error) {
	...
	return utils.Slice2MapSlice(handles, func(h *model.ResourceHandleTemplate) (int64, *model.ResourceHandleTemplate, bool) {
...
func (e *envImpl) GetResourceNodeTemplates(ctx context.Context, ids []int64) ([]*model.ResourceNodeTemplate, error) {
	...
	statement := e.DBWithContext(ctx).Select("id, uuid, name, config_schema").Where("id in ?", ids).Find(&datas)
	if statement.Error != nil {
...
func (e *envImpl) AddLabMember(ctx context.Context, datas ...*model.LaboratoryMember) error {
	...
	if statement.Error != nil {
		logger.Errorf(ctx, "AddLabMember err: %+v", statement.Error)
		return code.CreateDataErr.WithErr(statement.Error)
	}
...
func (e *envImpl) GetLabByUserID(ctx context.Context, req *common.PageReqT[string]) (*common.PageResp[[]*model.LaboratoryMember], error) {
	...
	return e.getLabMember(ctx, query, &req.PageReq)
}
...
func (e *envImpl) getLabMember(ctx context.Context, query *gorm.DB, req *common.PageReq) (*common.PageResp[[]*model.LaboratoryMember], error) {
	var total int64
	...
	if err := query.
		...
		Offset(req.Offest()).
		Limit(req.PageSize).
		Order("pin_time desc NULLS LAST, id asc").
		Find(&datas).Error; err != nil {
		logger.Errorf(ctx, "getLabMember fail err: %+v", err)
...
// 增加实验室默认内置角色，admin 和 member
func (e *envImpl) AddLabInnerRole(ctx context.Context, userID string, labID int64) error {
	customRoles := []*model.CustomRole{
		{
			LabID:    labID,
			RoleName: string(common.Admin),
		},
		{
			LabID:    labID,
			RoleName: string(common.Normal),
		},
	}

	if err := e.DBWithContext(ctx).Create(customRoles).Error; err != nil {
		logger.Errorf(ctx, "envImpl.AddLabInnerRole.CustomRole fail err: %+v", err)
		return err
	}

	if err := e.DBWithContext(ctx).Create(&model.UserRole{
		LabID:        labID,
		UserID:       userID,
		CustomRoleID: customRoles[0].ID,
	}).Error; err != nil {
		logger.Errorf(ctx, "envImpl.AddLabInnerRole.UserRole fail err: %+v", err)
		return err
	}
...
func (e *envImpl) AddUserRole(ctx context.Context, labID int64, roleIDs []int64, userID string) error {
	datas := utils.FilterSlice(roleIDs, func(id int64) (*model.UserRole, bool) {
	...
	if err := e.DBWithContext(ctx).Clauses(clause.OnConflict{
		Columns: []clause.Column{
			{Name: "lab_id"},
			{Name: "custom_role_id"},
			{Name: "user_id"},
		...
		logger.Errorf(ctx, "AddUserRole create role err: %+v", err)
