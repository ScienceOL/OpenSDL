...
type IDOrUUIDTranslate interface {
	...
	// 获取角色
	GetRole(ctx context.Context, labUUID uuid.UUID, userUUID string) (*model.LaboratoryMember, error)
}
...
func (b *Base) FindDatas(ctx context.Context, datas any, condition map[string]any, keys ...string) error {
	...
	if err := db.Order("id desc").Find(datas).Error; err != nil {
...
func (b *Base) CreateData(ctx context.Context, data schema.Tabler) error {
	if err := b.DBWithContext(ctx).Create(data).Error; err != nil {
		...
		if strings.Contains(errStr, "Duplicate entry") ||
			...
			return code.DataExistErr.WithErr(err)
		}
...
func (b *Base) GetRole(ctx context.Context, labUUID uuid.UUID, userID string) (*model.LaboratoryMember, error) {
	data := &model.LaboratoryMember{}
	if err := b.DBWithContext(ctx).Model(data).
		Joins("JOIN laboratory ON laboratory_member.lab_id = laboratory.id").
		Where("laboratory_member.user_id = ? AND laboratory.uuid = ?", userID, labUUID).
		First(data).Error; err != nil {
		logger.Errorf(ctx, "GetRole lab uuid: %s, user id: %s, err: %+v", labUUID, userID, err)
		return nil, code.LabMemberNotFoundErr
	}
