...
- The workspace's `registry/unilabos` directory is reachable. We pass
  ...
  example for clarity.
- For ESP-NOW recipes, the gateway board is plugged into a serial port
  (`/dev/cu.usbserial-A5069RR4` on this machine). Override with
  `--espnow-port` or `OSDL_ESPNOW_PORT`.
...
|---|---|---|
| [`runze-single-pump.md`](runze-single-pump.md) | One Runze syringe pump end-to-end (init, query, set valve) | ESP-NOW gateway + 1 Runze pump (legacy 1:1 path) |
...
| [`bus-manifest.md`](bus-manifest.md) | Validate the bus manifest by listing 5 registered devices behind one child | ChinWe |
...
- `espnow_probe.rs` — minimum viable gateway+child loopback test.
