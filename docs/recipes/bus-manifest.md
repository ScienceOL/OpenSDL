...
manifest, waits for the child to register, and prints the resulting
...
- Each has a `role` from the manifest (`syringe_pump` ×3, `stirrer`,
  `drain_valve`).
- Each shares the same `transport_id` (the child's MAC under `espnow:…`).

## Common failures

| Symptom | Cause |
|---|---|
| `device wait` times out | Child didn't REG — power, gateway port, or MAC mismatch |
| Only 1 device, id `espnow:<MAC>` (no `:pump-1` suffix) | `match_hardware_id` in your YAML doesn't match the child's announced `hardware_id` — engine fell back to the legacy 1:1 path |
