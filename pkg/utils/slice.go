//nolint:revive // var-naming: common package contains shared utilities
package utils

import "slices"

func AppendUniqSlice[T comparable](slice []T, elems ...T) []T {
	for _, elem := range elems {
		if slices.Contains(slice, elem) {
			continue
		}
		slice = append(slice, elem)
	}
	return slice
}

func FilterUniqSlice[T comparable, V comparable](slice []T, f func(elem T) (V, bool)) []V {
	res := make([]V, 0, len(slice))
	for _, v := range slice {
		if value, isAdd := f(v); isAdd && !slices.Contains(res, value) {
			res = append(res, value)
		}
	}

	return res
}

func Or[T comparable](values ...T) T {
	var zero T
	for _, v := range values {
		if v != zero {
			return v
		}
	}

	if len(values) > 0 {
		return values[len(values)-1]
	}

	return zero
}

// TernaryLazy 延迟计算版本的三元运算符，避免预先计算所有参数
// 只有在需要时才会调用相应的函数来计算值
func TernaryLazy[T any](condition bool, trueFn, falseFn func() T) T {
	if condition {
		return trueFn()
	}
	return falseFn()
}

func Ternary[T any](condition bool, okValue T, defaultValue T) T {
	if condition {
		return okValue
	}

	return defaultValue
}

func SafeValue[T any](f func() T, defaultVal T) (res T) {
	defer func() {
		if r := recover(); r != nil {
			res = defaultVal
		}
	}()

	return f()
}

func RemoveDuplicates[T comparable](slice []T) []T {
	keys := make(map[T]bool)
	result := []T{}

	for _, item := range slice {
		if !keys[item] {
			keys[item] = true
			result = append(result, item)
		}
	}

	return result
}

func FilterSlice[S any, T any](sources []T, f func(i T) (S, bool)) []S {
	newSlice := make([]S, 0, len(sources))
	for _, item := range sources {
		data, isAdd := f(item)
		if isAdd {
			newSlice = append(newSlice, data)
		}
	}
	return newSlice
}

func Range[T any](source []T, f func(index int, i T) bool) {
	for index, item := range source {
		if !f(index, item) {
			return
		}
	}
}

func RangeMap[T comparable, V any](source map[T]V, f func(key T, value V) bool) {
	for index, item := range source {
		if !f(index, item) {
			return
		}
	}
}

func FindTarget[S any, T any](sources []T, f func(i T) (S, bool)) S {
	var zero S
	for _, item := range sources {
		data, ok := f(item)
		if ok {
			return data
		}
	}
	return zero
}

func FilterSliceErr[S any, T any](sources []T, f func(i T) (S, bool, error)) ([]S, error) {
	newSlice := make([]S, 0, len(sources))
	for _, item := range sources {
		datas, isAdd, err := f(item)
		if err != nil {
			return nil, err
		}
		if isAdd {
			newSlice = append(newSlice, datas)
		}
	}
	return newSlice, nil
}

func FilterSliceWithErr[S any, T any](sources []T, f func(i T) ([]S, bool, error)) ([]S, error) {
	newSlice := make([]S, 0, len(sources))
	for _, item := range sources {
		datas, isAdd, err := f(item)
		if err != nil {
			return nil, err
		}
		if isAdd {
			newSlice = append(newSlice, datas...)
		}
	}
	return newSlice, nil
}

func Slice2Map[K comparable, V any, T any](sources []T, f func(i T) (K, V)) map[K]V {
	result := make(map[K]V)
	for _, item := range sources {
		key, value := f(item)
		result[key] = value
	}
	return result
}

func Slice2MapCondition[K comparable, V any, T any](sources []T, f func(i T) (K, V, bool)) map[K]V {
	result := make(map[K]V)
	for _, item := range sources {
		key, value, add := f(item)
		if !add {
			continue
		}
		result[key] = value
	}
	return result
}

func Slice2MapSlice[K comparable, V any, T any](sources []T, f func(i T) (K, V, bool)) map[K][]V {
	result := make(map[K][]V)
	for _, item := range sources {
		if key, value, isAdd := f(item); isAdd {
			result[key] = append(result[key], value)
		}
	}
	return result
}

func Map2Slice[K comparable, V any, T any](sources map[K]V, f func(key K, value V) (T, bool)) []T {
	result := make([]T, 0, len(sources))
	for k, v := range sources {
		data, add := f(k, v)
		if add {
			result = append(result, data)
		}
	}

	return result
}

// 语法糖包装
func Wrapper[T any](f func() T) T {
	return f()
}

func SetDifference[T comparable, V any](source map[T]V, target map[T]V) map[T]V {
	diffMap := make(map[T]V)
	for k, v := range source {
		if _, ok := target[k]; ok {
			continue
		}
		diffMap[k] = v
	}

	return diffMap
}

func FindDuplicates[E any, T comparable](slice []E, kf func(ele E) T) (uniques []E, duplicates []E) {
	seen := make(map[T]int)

	// 第一次遍历：统计每个元素的出现次数
	for _, item := range slice {
		seen[kf(item)]++
	}

	// 第二次遍历：根据出现次数分类
	for _, item := range slice {
		key := kf(item)
		if count, exists := seen[key]; exists {
			if count > 1 {
				duplicates = append(duplicates, item)
			} else {
				uniques = append(uniques, item)
			}
		}
	}

	return uniques, duplicates
}
